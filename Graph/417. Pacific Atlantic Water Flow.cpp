...
public:
    ...
    void DFS(vector<vector<int>>& heights, int i, int j, int prevCelVal,
        ...
        if (i < 0 || i >= heights.size() || j < 0 || j >= heights[0].size())
            return;
        if (heights[i][j] < prevCelVal || visited[i][j])
            return;
        visited[i][j] = true;
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        // DFS for 1st row and last row
        for (int i = 0; i < n; i++) {
            DFS(heights, 0, i, INT_MIN, pacificVisited);
            DFS(heights, m - 1, i, INT_MIN, atlanticVisited);
        }

        // DFS for 1st col and last col
        for (int j = 0; j < m; j++) {
            DFS(heights, j, 0, INT_MIN, pacificVisited);
            DFS(heights, j, n - 1, INT_MIN, atlanticVisited);
        }
